...
## Individual supported route (September 2026)

The landing form selects a separate supported route when the full configured name in `SUPPORTED.matches()` appears, ignoring case, repeated spaces, punctuation and full-width Latin characters. Extra class text before/after the name is accepted; a first name or surname alone does not match. This is a local presentation preference, not an identity or access-control system.

The standard lesson content and its renderers are unchanged. `supported.js` supplies short English/Simplified Chinese steps for the same message-checker topic: six starter checks, data and names, arithmetic/comparison/Boolean operators, three trace exercises, challenges, separate starting/end reflections and PDF hand-in. Learners choose reasons, complete short code lines, select trace cells and edit runnable Python examples. Short English or Chinese responses are accepted. Feedback and tutor use are recorded; selected answers are not labelled as independent skill. Learning phases come from the learner's choices, not correctness.

Supported notebooks use the existing student/teacher storage key plus `-guided`. The normal notebook is saved before switching routes and restored when a nonmatching name is entered. A small `-route` preference reopens the most recently used notebook on reload. Existing standard answers and version-1 backups remain compatible. Name/class and the selected route are derived again when a backup is restored. Reports contain supported responses, first attempts, code/output and reflection evidence, rather than unused standard questions. An active Python worker is stopped before a notebook switch or restore.

Implementation files: `supported.js`, `supported.css`, a loader in `index.html` and narrow routing/storage/report hooks in `app.js`. Keep these together when publishing. No external service receives the name or answers. The configured name should be maintained only by the teacher; learner pages use the neutral label “One step at a time”.

Validation: all 54 standard page/language/view renders matched the original, 180 supported-step render cases passed, and name matching, notebook switching/reload, backup validation, first attempts, summaries and report escaping were checked. Ten completed-scaffold/input cases passed through the bundled Python worker. Live browser checks confirmed uppercase-name routing, bilingual MCQ feedback, step navigation and a Python run producing 3 then 2.
