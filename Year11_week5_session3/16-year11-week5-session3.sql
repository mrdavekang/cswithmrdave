-- Step 16: enable normal Year 11 Week 5 Session 3 classroom pages.
-- Teacher presentation slides are deliberately NOT valid student destinations.
...
returns boolean language sql immutable set search_path='' as $$
 ...
 when 'Year11_week5_session3' then p_stage in ('read','start','before','main1','sql','main2','pit','exit','extension','mark','submit')
 else false end,false);
$$;
revoke all on function classroom_private.valid_lesson_stage(text,text) from public,anon,authenticated;
commit;
select classroom_private.valid_lesson_stage('Year11_week5_session3','read') as session3_classroom_ready;
