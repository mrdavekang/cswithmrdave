...
function update(){const now=clock.at(clock.seconds()),info=window.LessonClassroom?.info(),left=Math.max(0,Math.ceil(now.remaining)),display=String(Math.floor(left/60)).padStart(2,'0')+':'+String(left%60).padStart(2,'0');
 show('clock-time',clock.time(Math.floor(clock.seconds()/60)));show('clock-bubble-stage',following?(now.phase==='before'?'Starts 11:20':now.phase==='after'?'Save & submit':now.stage.title):'Clock only');
...
let corner='br';try{const saved=localStorage.getItem('y11w5s3-clock-corner');if(['tl','tr','bl','br'].includes(saved))corner=saved;}catch{}
function place(value){corner=value;root.dataset.corner=value;root.style.left='';root.style.top='';root.style.right='';root.style.bottom='';try{localStorage.setItem('y11w5s3-clock-corner',value);}catch{}}
place(corner);
