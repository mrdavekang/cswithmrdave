(() => {
  ...
  const reportFields = [
    {
      title: "Starter · Connected or stand-alone?",
      completed: "starter",
      fields: [
        ["Embedded-system retrieval", "starter_embedded"],
        ["Connected devices and evidence", "starter_connected"],
        ["Two shared resources", "starter_shared"],
        ["Predicted benefit", "starter_benefit"],
        ["Predicted risk", "starter_risk"],
        ["Provisional network definition", "starter_definition"],
      ],
    },
    {
      title: "Main Activity 1 · Read, classify, develop",
      completed: "main1",
      fields: [
        ["Revised network definition", "main1_definition"],
        ["Evidence 1 classification", "evidence_1"],
        ["Evidence 2 classification", "evidence_2"],
        ["Evidence 3 classification", "evidence_3"],
        ["Evidence 4 classification", "evidence_4"],
        ["Evidence 5 classification", "evidence_5"],
        ["Evidence 6 classification", "evidence_6"],
        ["Evidence 7 classification", "evidence_7"],
        ["Evidence 8 classification", "evidence_8"],
        ["Evidence 9 classification", "evidence_9"],
        ["Evidence 10 classification", "evidence_10"],
        ["Developed benefit", "main1_benefit"],
        ["Developed risk", "main1_risk"],
        ["Improved response", "main1_improvement"],
      ],
    },
    {
      title: "Main Activity 2 · Exam-office decision",
      completed: "main2",
      fields: [
        ["Three network uses", "main2_uses"],
        ["Contextual benefit 1", "main2_benefit1"],
        ["Contextual benefit 2", "main2_benefit2"],
        ["Contextual risk 1", "main2_risk1"],
        ["Contextual risk 2", "main2_risk2"],
        ["Balanced recommendation", "main2_response"],
        ["Improved weakest sentence", "main2_improvement"],
      ],
    },
    {
      title: "Past-paper extension · June 2025 Paper 2 Q10.4",
      completed: "extension",
      fields: [
        ["Response to Q10.4", "extension_response"],
      ],
    },
    {
      title: "Plenary · Three-sentence exit ticket",
      completed: "plenary",
      fields: [
        ["Sentence 1: network definition", "plenary_definition"],
        ["Sentence 2: developed benefit", "plenary_benefit"],
        ["Sentence 3: developed risk", "plenary_risk"],
      ],
    },
  ];
  ...
  function makeStorageKey(profile) {
    return `year11-networks-w2s1-v1:${safePart(profile.role)}:${safePart(profile.name)}:${safePart(profile.className)}`;
  }
  ...
  function saveState() {
    ...
    localStorage.setItem("year11-networks-w2s1-last-profile", JSON.stringify(state.profile));
  ...
  function exportPdf() {
    ...
    const pageHeader = (label = "YEAR 11 COMPUTER SCIENCE · COMPUTER NETWORKS") => {
      doc.setDrawColor(0);
      doc.setTextColor(0);
    ...
    doc.setFontSize(26);
    doc.text("Computer Networks", margin, 38);
    doc.setFontSize(17);
    ...
    response("WAGBA", "Developing network points into precise, contextual exam explanations.");
    response("Core lesson progress", `${completionPercent()}% complete`);
    wrapped("ASSESSMENT-OBJECTIVE JOURNEY", 8, "bold");
    wrapped("AO1: define a computer network and identify common uses, benefits and risks. AO2: apply knowledge to a specific organisation and explain mechanisms and consequences.", 9.2);
    y += 3;
    ...
    const filename = `${safePart(state.profile.name)}_${safePart(state.profile.className)}_computer-networks-learning-report.pdf`;
    doc.save(filename);
  ...
  try {
    const last = JSON.parse(localStorage.getItem("year11-networks-w2s1-last-profile"));
