...
const root=document.createElement('details');root.id='lesson-clock';root.innerHTML=`<summary title="Drag to move; click to open lesson clock"><span id="clock-time"></span><b id="clock-short"></b><span id="clock-bubble-stage"></span></summary><div class="clock-panel"><h2>2:00–3:00 p.m.</h2><p class="clock-note">Malaysia time · uses your device clock</p><label><input id="clock-follow" type="checkbox" checked> Follow lesson clock</label><p class="clock-note">This suggests an activity. It never moves you automatically.</p><div id="clock-guidance"><h3 id="clock-stage"></h3><strong id="clock-countdown"></strong><p id="clock-current"></p><p id="clock-next"></p><button id="clock-open">Open suggested page</button></div><fieldset class="clock-corners"><legend>Move clock to a corner</legend><button type="button" data-corner="tl">↖ Top left</button><button type="button" data-corner="tr">↗ Top right</button><button type="button" data-corner="bl">↙ Bottom left</button><button type="button" data-corner="br">↘ Bottom right</button></fieldset><h3>Today's plan</h3><ol>${plan.stages.map(s=>`<li><b>${clock.time(s.start)}–${clock.time(s.end)}</b> ${s.title} <small>(${s.end-s.start} min)</small></li>`).join('')}</ol><p class="clock-note">Extra-time threshold and sound challenges fit within task time when your teacher directs you. Keep working at your own pace when needed.</p></div>`;document.body.append(root);
let following=true;try{following=localStorage.getItem('y8w5project-follow-clock')!=='off';}catch{}const check=document.getElementById('clock-follow');check.checked=following;
check.onchange=()=>{following=check.checked;try{localStorage.setItem('y8w5project-follow-clock',following?'on':'off');}catch{}update();};
...
function update(){const now=clock.at(clock.seconds()),info=window.LessonClassroom?.info(),left=Math.max(0,Math.ceil(now.remaining)),display=String(Math.floor(left/60)).padStart(2,'0')+':'+String(left%60).padStart(2,'0');
 show('clock-time',clock.time(Math.floor(clock.seconds()/60)));show('clock-bubble-stage',following?(now.phase==='before'?'Starts 14:00':now.phase==='after'?'Save & submit':now.stage.title):'Clock only');
 ...
 show('clock-countdown',now.phase==='before'?display+' until 14:00':now.phase==='after'?'Save any remaining work':display+' remaining');
...
let corner='br';try{const saved=localStorage.getItem('y8w5project-clock-corner');if(['tl','tr','bl','br'].includes(saved))corner=saved;}catch{}
function place(value){corner=value;root.dataset.corner=value;root.style.left='';root.style.top='';root.style.right='';root.style.bottom='';try{localStorage.setItem('y8w5project-clock-corner',value);}catch{}}
place(corner);
