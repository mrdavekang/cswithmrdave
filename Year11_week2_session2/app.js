(() => {
  ...
  const reportFields = [
    {
      title: "Starter · From one room to the world",
      completed: "starter",
      fields: [
        ["Network definition", "starter_definition"],
        ["Predicted benefit", "starter_benefit"],
        ["Predicted risk", "starter_risk"],
        ["Smallest-to-largest order", "starter_order"],
        ["Fixed-desktop prediction", "starter_desktops"],
        ["Mobile-tablet prediction", "starter_tablets"],
      ],
    },
    {
      title: "Main Activity 1 · Classify without guessing",
      completed: "main1",
      fields: [
        ["Case A: scale", "case_a_scale"],
        ["Case A: method", "case_a_method"],
        ["Case A: evidence", "case_a_evidence"],
        ["Case B: scale", "case_b_scale"],
        ["Case B: method", "case_b_method"],
        ["Case B: evidence", "case_b_evidence"],
        ["Case C: scale", "case_c_scale"],
        ["Case C: method", "case_c_method"],
        ["Case C: evidence", "case_c_evidence"],
        ["Case D: scale", "case_d_scale"],
        ["Case D: method", "case_d_method"],
        ["Case D: evidence", "case_d_evidence"],
        ["Correction: every LAN is wired", "main1_correction1"],
        ["Correction: every wireless network is a WAN", "main1_correction2"],
        ["Improved classification or correction", "main1_improvement"],
      ],
    },
    {
      title: "Main Activity 2 · Network architect challenge",
      completed: "main2",
      fields: [
        ["Administration: scale", "admin_scale"],
        ["Administration: method", "admin_method"],
        ["Administration: medium", "admin_medium"],
        ["Administration: justification", "admin_justification"],
        ["Media building: scale", "media_scale"],
        ["Media building: method", "media_method"],
        ["Media building: medium", "media_medium"],
        ["Media building: justification", "media_justification"],
        ["Tablets: scale", "tablet_scale"],
        ["Tablets: method", "tablet_method"],
        ["Tablets: medium", "tablet_medium"],
        ["Tablets: justification", "tablet_justification"],
        ["Second campus: classification", "campus_scale"],
        ["Second campus: conclusion limits", "campus_justification"],
        ["Complete network recommendation", "main2_recommendation"],
        ["Improved weakest sentence", "main2_improvement"],
      ],
    },
    {
      title: "Past-paper extension · November 2025 Paper 2 Q07",
      completed: "extension",
      fields: [
        ["07.1 response", "extension_q071"],
        ["07.2 Difference 1", "extension_q072a"],
        ["07.2 Difference 2", "extension_q072b"],
        ["07.3 response", "extension_q073"],
      ],
    },
    {
      title: "Plenary · Three decisions",
      completed: "plenary",
      fields: [
        ["Sentence 1: school Wi-Fi and LAN", "plenary_lan"],
        ["Sentence 2: wireless choice and trade-off", "plenary_wireless"],
        ["Sentence 3: fibre choice", "plenary_fibre"],
      ],
    },
  ];
  ...
  function makeStorageKey(profile) {
    return `year11-networks-w2s2-v1:${safePart(profile.role)}:${safePart(profile.name)}:${safePart(profile.className)}`;
  }
  ...
  function saveState() {
    ...
    localStorage.setItem("year11-networks-w2s2-last-profile", JSON.stringify(state.profile));
  ...
  function completeSection(section) {
    ...
    if (incomplete) {
      ...
      showToast(incomplete.dataset?.requiredGroup ? "Complete the required selections." : "Complete every required response before moving on.");
      return;
    }
  ...
  function exportPdf() {
    ...
    const pageHeader = (label = "YEAR 11 COMPUTER SCIENCE · NETWORK CHOICES") => {
      doc.setDrawColor(0);
      doc.setTextColor(0);
    ...
    doc.setFontSize(26);
    doc.text("Choosing the Right Network", margin, 38);
    doc.setFontSize(17);
    ...
    response("WAGBA", "Selecting and justifying an appropriate network type and transmission medium from scenario evidence.");
    response("Core lesson progress", `${completionPercent()}% complete`);
    wrapped("ASSESSMENT-OBJECTIVE JOURNEY", 8, "bold");
    wrapped("AO1: describe LAN, WAN, wired, wireless, copper twisted pair and fibre optic. AO2: apply their characteristics to unfamiliar requirements and justify suitable choices.", 9.2);
    y += 3;
    ...
    const filename = `${safePart(state.profile.name)}_${safePart(state.profile.className)}_network-choices-learning-report.pdf`;
    doc.save(filename);
  ...
  $("#student-entry").addEventListener("submit", (event) => {
    ...
    if (!name || !className) {
      showToast("Enter your full name and class to begin.");
      return;
    }
  ...
  try {
    const last = JSON.parse(localStorage.getItem("year11-networks-w2s2-last-profile"));
